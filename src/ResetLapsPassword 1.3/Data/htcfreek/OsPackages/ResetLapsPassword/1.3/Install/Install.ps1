...
function Invoke-LapsResetCommands([PSCustomObject]$LapsResetTasks, [bool]$DoResetImmediately, [bool]$ForceResetOnAzureTarget)
{
    ...
    #    - [bool]$ForceResetOnAzureTarget : $true = Reset anyway.; $false = Do not reset again if already done by system. (With Azure AD as Windows LAPS target.)

    # Debug/Log information
    WriteLogDebug "Starting reset sequence ..."
    WriteLogDebug "Final reset task summary: $($LapsResetTasks -replace ';',',' -replace '@{','' -replace '}',',') DoResetImmediately=$($DoResetImmediately), ForceResetOnAzureTarget=$($ForceResetOnAzureTarget)"
    ...
    if ($LapsResetTasks.WinLaps)
    {
        ...
        try
        {
            if ($LapsResetTasks.WinLapsIsAzureTarget -or $DoResetImmediately)
            {
                ...
                if ($LapsResetTasks.WinLapsIsAzureTarget -and $isAzureExpTime -and ($ForceResetOnAzureTarget -eq $false))
                {
                ...
                else
                {
                    ...
                    if ($LapsResetTasks.WinLapsIsAzureTarget -and $ForceResetOnAzureTarget)
                    {
...
function Main() {
    ...
    [bool]$varForceResetOnAzureTarget = ReadEmpirumVariable -varName ResetLapsPassword.ForceResetOnAzureTarget -isBoolean -defaultValue 0
    ...
    Invoke-LapsResetCommands -LapsResetTasks $lapsResetTasks -DoResetImmediately $varLapsResetImmediately -ForceResetOnAzureTarget $varForceResetOnAzureTarget
